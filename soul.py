...
TOKEN = '7436972230:AAHjfU0GNwwzgPm8iMKofusSEhZLHWfBwrc'
...
def approve_or_disapprove_user(message):
    ...
    if not is_admin:
        bot.send_message(chat_id, "*You are not authorized to use this command please contact with @Felixbbyy*", parse_mode='Markdown')
        return
...
def attack_command(message):
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "You are not approved to use this bot. Please contact with @Felixbbyy 🩵🫶.")
            return
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*You are not approved to use this bot. Please contact with @Felixbbyy 🩷🥹.*", parse_mode='Markdown')
            return
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "*Mod By • @Felixbbyy • Choose an option:*", reply_markup=markup, parse_mode='Markdown')
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        if user_data:
            ...
            response = "*No account information found. Please contact with @Felixbbyy 🩵🫶.*"
